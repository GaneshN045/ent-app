    ...
    return (
        <ScrollView className="flex-1">
            ...
            <View className=" bg-[#F7F8FA] px-5 pt-6">
                ...
                <View className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-5">
                    {deviceOptions.map((item, index) => (
                        <TouchableOpacity
                            ...
                            className={`flex-row items-center px-4 py-3 mb-2 rounded-xl border ${device === item
                        ...
                        >
                            <MaterialIcons
                                name={
                                    device === item ? "radio-button-checked" : "radio-button-unchecked"
                                }
                                size={22}
                            ...
                            <Text
                                className={`ml-3 text-base ${device === item ? "text-red-600 font-semibold" : "text-gray-700"
                                    }`}
                            >
                                {item}
                            </Text>
                ...
                {/* CSP Checkbox */}
                <TouchableOpacity
                    onPress={() => setIsCSP(!isCSP)}
                    className="flex-row items-center mb-6"
                >
                    <View
                        className={`w-6 h-6 rounded-md border mr-3 ${isCSP ? "bg-red-600 border-red-600" : "border-gray-400"
                            }`}
                    >
                        {isCSP && (
                            <MaterialIcons
