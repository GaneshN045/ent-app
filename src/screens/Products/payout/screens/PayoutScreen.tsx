...
export default function PayoutScreen() {
  ...
  const maskAccountNo = (accNo: string) => (accNo.length <= 4 ? accNo : `XXXX ${accNo.slice(-4)}`);
  ...
  const handleAddAccount = (newAcc: Omit<BankAccount, 'id' | 'verified'>) => {
    setLoading(true);
    setTimeout(() => {
      setAccounts(prev => [...prev, { ...newAcc, id: prev.length + 1, verified: false }]);
      setLoading(false);
  ...
  const handleVerify = (id: number) => {
    setLoading(true);
    setTimeout(() => {
      setAccounts(prev => prev.map(a => (a.id === id ? { ...a, verified: true } : a)));
      setLoading(false);
  ...
  const handleTransfer = (id: number, type: string, amount: string) => {
    ...
    setTimeout(() => {
      ...
      Alert.alert('Transfer Success', `₹${amount} sent to ${acc?.holderName} via ${type}`);
      setLoading(false);
    }, 1500);
  };
